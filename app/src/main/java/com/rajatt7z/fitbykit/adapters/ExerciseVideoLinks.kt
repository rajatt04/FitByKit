...
/**
 * Maps exercise names (as returned by the WGER API) to curated YouTube video URLs.
 *
 * IMPORTANT: Each key must be unique. Previously, several names appeared in multiple
 * muscle-group sections; the duplicate entries have been deduplicated — the most
 * relevant / highest-quality video URL is kept for each exercise name.
 */
object ExerciseVideoLinks {
    val exerciseVideoMap = mapOf(
